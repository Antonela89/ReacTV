...
const CardDetalle = ({ movie, trailer }) => {
	...
	const handleList = async () => {
		...
		const route = collection(
			db,
			`Usuarios/${userEmailValue}/ListaPeliculas`,
		);
		...
		if (info.original_title) {
			...
			await handleListOrRecent(
				route,
				dataToAdd,
				await getDocs(query(route, where('id', '==', info.id))),
			);
		} else {
			...
			await handleListOrRecent(
				route2,
				dataToAdd,
				await getDocs(query(route2, where('id', '==', info.id))),
			);
		}
	};
	...
	const handleRecent = async () => {
		...
		const route3 = collection(
			db,
			`Usuarios/${userEmailValue}/RecentPeliculas`,
		);
		const route4 = collection(
			db,
			`Usuarios/${userEmailValue}/RecentSeries`,
		);
		...
		if (info.original_title) {
			...
			await handleListOrRecent(
				route3,
				dataToAdd2,
				await getDocs(query(route3, where('id', '==', info.id))),
			);
		} else {
			...
			await handleListOrRecent(
				route4,
				dataToAdd2,
				await getDocs(query(route4, where('id', '==', info.id))),
			);
		}
	};
	...
	const ChangeLanguage = ({ lenguaje }) => {
		...
		return (
			<p className="idioma original">
				IDIOMA ORIGINAL: {lenguajeOriginal}
			</p>
		);
	};
	...
	const GenerosListado = ({ generos }) => {
		return (
			<ul className="listado-generos">
				...
				{generos.map((genero) => (
					<li className="generos" key={genero.id}>
						{genero.name}
					</li>
				))}
			</ul>
	...
	const Anio = () => {
		const fechaCompleta =
			info && (info.first_air_date || info.release_date);
		if (!fechaCompleta) {
	...
	const Card = () => {
		const baseUrl = IMAGE_PATH || 'https://image.tmdb.org/t/p/w500';
		const path = info.backdrop_path || info.poster_path;
		const finalUrl = `${baseUrl}${path}`;

		// const finalUrl = `https://reactvserver--reactvstream.repl.co/imagen-proxy?imageUrl=${baseUrl}${path}`;
